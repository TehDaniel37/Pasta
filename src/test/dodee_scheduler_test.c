...
static void setup() 
{
    dodee_scheduler_set_exec(mock_exec_will_exit_unsuccessfully);
}

static void teardown()
{
    dodee_scheduler_kill_children();
    dodee_scheduler_reset_exec();
    dodee_scheduler_reset_forker();
    dodee_scheduler_reset_sleeper();
}

static void start_task_should_call_exec_with_correct_params()
{
    ...
    *mock_exec_correct_params = false; 
    ...
    DodeeTask task = { .name = "Test", .command = mock_exec_expected_params, .interval_seconds = 0, .state = Stopped };
    dodee_scheduler_set_exec(mock_exec_will_verify_params);

    dodee_scheduler_start_task(&task);
...
static void start_task_should_return_success_when_task_starts_successfully()
{
    DodeeTask task = { .name = "Test", .command = "echo hello", .interval_seconds = 0, .state = Stopped };

    DodeeStatus status = dodee_scheduler_start_task(&task);

    ssct_assert_equals(status, DODEE_SUCCESS);
}

static void start_task_should_set_task_state_to_running()
{
    DodeeTask task = { .name = "Test", .command = "echo hello", .interval_seconds = 0, .state = Stopped };

    dodee_scheduler_start_task(&task);

    ssct_assert_equals(task.state, Running);
}

static void start_task_should_return_fork_failed_error()
{
    DodeeTask task = { .name = "Test", .command = "ehco", .interval_seconds = 1, .state = Stopped };

    dodee_scheduler_set_forker(mock_fork_will_fail);
    DodeeStatus status = dodee_scheduler_start_task(&task);
    
    ssct_assert_equals(status, DODEE_ERROR_FORK_FAILED);
}

static void start_task_should_call_exec_repeatedly()
{
    DodeeTask task = { .name = "Test", .command = "echo", .interval_seconds = 0, .state = Stopped };
    times_exec_called = (int *)create_shared_memory(sizeof (int));
    *times_exec_called = 0;
    
    dodee_scheduler_set_exec(mock_exec_will_count_times_called);
...
static void start_task_should_pass_3600_seconds_to_sleep()
{
    DodeeTask task = { .name = "Test", .command = "echo", .interval_seconds = mock_sleep_expected_param, .state = Stopped };
    
    mock_sleep_correct_params = (bool *)create_shared_memory(sizeof (bool));
    
    dodee_scheduler_set_sleeper(mock_sleep);
    dodee_scheduler_start_task(&task);
...
static void start_task_should_exec_executable_file_with_absolute_path()
{
    DodeeTask task = { .name = "Test", .command = "/home/danalm/git/dodee/res/debug/test/test_script.sh", .interval_seconds = 0, .state = Stopped };
    ...
    *mock_exec_file_is_executable = false;
    ...
    dodee_scheduler_set_exec(mock_exec_will_check_if_file_exists_and_is_executable);
...
static void start_task_should_exec_executable_file_with_relative_path()
{
    system("mkdir -p $HOME/.config/dodee/bin");
    system("cp res/debug/test/test_script.sh $HOME/.config/dodee/bin");
    DodeeTask task = { .name = "Test", .command = "test_script.sh", .interval_seconds = 0, .state = Stopped };
...
{
    const char config_dir[] = "/home/danalm/.config/dodee";
    const char config_bin_dir[] = "/home/danalm/.config/dodee/bin";
...
static void stop_task_should_set_state_to_stopped()
{
    DodeeTask task = { .name = "Test", .command = "echo", .interval_seconds = 0, .state = Running };
    
    dodee_scheduler_stop_task(&task);
    
    ssct_assert_equals(task.state, Stopped);
}

static void stop_task_should_stop_process_associated_with_task()
{
    DodeeTask task = { .name = "Test", .command = "echo", .interval_seconds = 0, .state = Running };
    ...
    else
    {
        ...
        dodee_scheduler_associate_pid_with_task(&task, child_pid);
        dodee_scheduler_stop_task(&task);
...
int main(void)
{
    system("rm -rf $HOME/.config/dodee");
    ...
    ssct_run(start_task_should_call_exec_with_correct_params);
    ssct_run(start_task_should_return_success_when_task_starts_successfully);
    ssct_run(start_task_should_set_task_state_to_running);
    ssct_run(start_task_should_return_fork_failed_error);
    ssct_run(start_task_should_call_exec_repeatedly);
    ssct_run(start_task_should_pass_3600_seconds_to_sleep);
    ssct_run(start_task_should_exec_executable_file_with_absolute_path);
    ssct_run(start_task_should_exec_executable_file_with_relative_path);
    
    ssct_run(stop_task_should_set_state_to_stopped);
    ssct_run(stop_task_should_stop_process_associated_with_task);
